...
# Basic dataset configuration for ETTm2
DATASET_CONFIGS = {
    "ETTm2": {
        "data_path": "ETTm2.csv",
        "seq_len": 512,
...
def get_output_json_path(output_dir, gate_type, pred_len=96, density_mode=None):
    ...
    return os.path.join(output_dir, f"best_nooc_linear_ettm2_{gate_str}_{pred_len}{suffix}.json")


def get_storage_name(gate_type, pred_len=96, output_dir=None):
    db_name = f"best_ettm2_router_{pred_len}.db" if gate_type == "adaptive_direction" else f"best_ettm2_{gate_type if gate_type is not None else 'none'}_{pred_len}.db"
    if output_dir:
...
def get_study_name(gate_type, pred_len=96):
    if gate_type == "adaptive_direction":
        return f"best_ettm2_router_{pred_len}_v1"
    gate_str = gate_type if gate_type is not None else "none"
    return f"best_nooc_linear_ettm2_{gate_str}_{pred_len}_v1"
...
def sample_params(trial, bounds, position, gate_type=None, batch_size=512):
    ...
    gs_dropout = trial.suggest_float("gs_dropout", bounds["gs_dropout_lower"], bounds["gs_dropout_upper"], step=0.05)
    params = {
        "patch_len": 24,
        "stride": 12,
...
def build_command(position, params, trial_number, density_mode="cas", pred_len=96, gate_type=None, gate_beta=0.25, output_dir="loss_cas_simplify"):
    ...
    model_id = f"tune_linear_ettm2_gate_{gate_str}_trial_{trial_number}"
...
def load_seed_params(position, experiment_tag="simplify", pred_len=96, gate_type=None, batch_size=512):
    seed_path = os.path.join("tune_seed", f"best_ettm2_router_{pred_len}_cas.json")
    ...
    seed_trial["gs_residual_weight"] = seed_trial.get("gs_residual_weight", 0.1984)
    seed_trial["gate_beta"] = seed_trial.get("gate_beta", 0.25)
    
    dropout_val = seed.get("gs_dropout", seed.get("dropout", 0.55))
...
def enqueue_seed(study, seed_trial, seed_path, position):
    ...
    print(f"[Seed] Enqueued {seed_path} for ETTm2/{position}")
def main():
    parser = argparse.ArgumentParser(description="Ablation HPO script for ETTm2 gating choices.")
    ...
    parser.add_argument("--batch_size", type=int, default=1024, choices=[16, 32, 128, 256, 512, 1024], help="Fixed batch size for HPO (default: 1024)")
