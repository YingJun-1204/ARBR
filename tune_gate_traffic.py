...
# Basic dataset configuration for traffic
DATASET_CONFIGS = {
    "traffic": {
...
def get_output_json_path(output_dir, pred_len=96, density_mode=None):
    ...
    return os.path.join(output_dir, f"best_traffic_jet_{pred_len}{suffix}.json")


def get_storage_name(pred_len=96, output_dir=None):
    db_name = f"best_traffic_jet_{pred_len}.db"
    if output_dir:
...
def get_study_name(pred_len=96):
    return f"best_traffic_jet_{pred_len}_v1"
...
def build_command(position, params, trial_number, density_mode="cas", pred_len=96, output_dir="loss_cas_simplify", jet_derivative_mode="exact", use_scale_jet=False, ablation_mode="none"):
    ...
    cfg = DATASET_CONFIGS["traffic"]
    head_dropout = 0.0 if position == "none" else params["head_dropout"]
    model_id = f"tune_traffic_jet_trial_{trial_number}"
...
def make_objective(position, bounds, density_mode, pred_len, output_dir, batch_size=256, jet_derivative_mode="exact", use_scale_jet=False, jet_max_shift_samples=1.0, ablation_mode="none"):
    def objective(trial):
...
def load_seed_params(position, experiment_tag="simplify", pred_len=96, batch_size=8):
    seed_path = os.path.join("tune_seed", f"best_traffic_jet_{pred_len}_cas.json")
    ...
    seed_trial["batch_size"] = seed_trial.get("batch_size", 8)
...
def enqueue_seed(study, seed_trial, seed_path, position):
    ...
    print(f"[Seed] Enqueued {seed_path} for traffic/{position}")


def main():
    parser = argparse.ArgumentParser(description="Ablation HPO script for traffic jet tuning.")
    parser.add_argument("--pred_len", type=int, nargs="+", default=[96], help="Prediction horizons (default: [96])")
    parser.add_argument("--batch_size", type=int, default=8, choices=[8, 16, 32, 128, 256, 512, 1024], help="Fixed batch size for HPO (default: 8)")
    ...
    parser.add_argument("--ablation_mode", type=str, default="none", choices=["none", "gaussian_only", "observation_only", "wo_ajc", "wo_jet"], help="Ablation mode selection")
