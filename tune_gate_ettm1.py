...
# Basic dataset configuration for ETTm1
DATASET_CONFIGS = {
    "ETTm1": {
        "data_path": "ETTm1.csv",
        "seq_len": 512,
...
# Initial tuning search bounds for HPO centered around ETTm1 baseline
DEFAULT_BOUNDS = {
    "learning_rate_lower": 1e-4,
    "learning_rate_upper": 1.5e-3,
...
def get_output_json_path(output_dir, gate_type, pred_len=96, density_mode=None):
    ...
    if gate_type == "adaptive_direction":
        return os.path.join(output_dir, f"best_ettm1_router_{pred_len}{suffix}.json")
    gate_str = gate_type if gate_type is not None else "none"
    return os.path.join(output_dir, f"best_nooc_linear_ettm1_{gate_str}_{pred_len}{suffix}.json")


def get_storage_name(gate_type, pred_len=96, output_dir=None):
    db_name = f"best_ettm1_router_{pred_len}.db" if gate_type == "adaptive_direction" else f"best_ettm1_{gate_type if gate_type is not None else 'none'}_{pred_len}.db"
    if output_dir:
...
def get_study_name(gate_type, pred_len=96):
    if gate_type == "adaptive_direction":
        return f"best_ettm1_router_{pred_len}_v1"
    gate_str = gate_type if gate_type is not None else "none"
    return f"best_nooc_linear_ettm1_{gate_str}_{pred_len}_v1"
...
def sample_params(trial, bounds, position, gate_type=None, batch_size=512):
...
def build_command(position, params, trial_number, density_mode="cas", pred_len=96, gate_type=None, gate_beta=0.25, output_dir="loss_cas_simplify"):
    ...
    model_id = f"tune_linear_ettm1_gate_{gate_str}_trial_{trial_number}"
...
def make_objective(position, bounds, density_mode, pred_len, gate_type, gate_beta, output_dir, batch_size=512):
    def objective(trial):
...
def save_best_callback(position, output_dir, gate_type, density_mode, pred_len, num_gaussians=None):
    def callback(study, trial):
        ...
        if should_write:
            ...
            best_tuned["batch_size"] = int(best_params.get("batch_size", 512))
...
def load_seed_params(position, experiment_tag="simplify", pred_len=96, gate_type=None, batch_size=512):
    seed_path = os.path.join("tune_seed", f"best_ettm1_router_{pred_len}_cas.json")
    ...
    seed_trial["gs_residual_weight"] = seed_trial.get("gs_residual_weight", 0.2)
    seed_trial["gate_beta"] = seed_trial.get("gate_beta", 0.4)
    
    dropout_val = seed.get("gs_dropout", seed.get("dropout", 0.45))
    seed_trial["gs_dropout"] = float(dropout_val)
    
    if gate_type == "adaptive_direction":
        seed_trial["gate_lambda"] = seed_trial.get("gate_lambda", seed.get("gate_lambda", 0.1))
...
def enqueue_seed(study, seed_trial, seed_path, position):
    ...
    print(f"[Seed] Enqueued {seed_path} for ETTm1/{position}")
def main():
    parser = argparse.ArgumentParser(description="Ablation HPO script for ETTm1 gating choices.")
    ...
    # Parse gate_type string to Python None if "none"
    gate_val = None if args.gate_type == "none" else args.gate_type

    # 2. HPO mode run
    for p_len in args.pred_len:
        print(f"\n=== [MODE: HPO {args.gate_type.upper()}] Starting tuning for pred_len={p_len} ===")
        bounds = make_bounds()
