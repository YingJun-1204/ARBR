...
def get_output_json_path(output_dir, pred_len=96, density_mode=None):
    ...
    return os.path.join(output_dir, f"best_etth2_jet_{pred_len}{suffix}.json")


def get_storage_name(pred_len=96, output_dir=None):
    db_name = f"best_etth2_jet_{pred_len}.db"
    if output_dir:
...
def get_study_name(pred_len=96):
    return f"best_etth2_jet_{pred_len}_v1"
...
def build_command(position, params, trial_number, density_mode="cas", pred_len=96, output_dir="loss_cas_simplify", jet_derivative_mode="exact", use_scale_jet=False, ablation_mode="none"):
    ...
    model_id = f"tune_etth2_jet_trial_{trial_number}"
...
def load_seed_params(position, experiment_tag="simplify", pred_len=96, batch_size=256):
    seed_path = os.path.join("tune_seed", f"best_etth2_jet_{pred_len}_cas.json")
...
def main():
    parser = argparse.ArgumentParser(description="Ablation HPO script for ETTh2 jet tuning.")
