...
def get_output_json_path(output_dir, gate_type, pred_len=96, density_mode=None):
    ...
    if gate_type == "adaptive_direction":
        return os.path.join(output_dir, f"best_weather_router_{pred_len}{suffix}.json")
    gate_str = gate_type if gate_type is not None else "none"
    return os.path.join(output_dir, f"best_nooc_linear_weather_{gate_str}_{pred_len}{suffix}.json")


def get_storage_name(gate_type, pred_len=96, output_dir=None):
    db_name = f"best_weather_router_{pred_len}.db" if gate_type == "adaptive_direction" else f"best_weather_{gate_type if gate_type is not None else 'none'}_{pred_len}.db"
    if output_dir:
...
def get_study_name(gate_type, pred_len=96):
    if gate_type == "adaptive_direction":
        return f"best_weather_router_{pred_len}_v1"
    gate_str = gate_type if gate_type is not None else "none"
    return f"best_nooc_linear_weather_{gate_str}_{pred_len}_v1"
...
def build_command(position, params, trial_number, density_mode="sparse", pred_len=96, gate_type=None, gate_beta=0.25, output_dir="loss_cas_simplify"):
    ...
    model_id = f"tune_linear_weather_gate_{gate_str}_trial_{trial_number}"
...
def load_seed_params(position, experiment_tag="simplify", pred_len=96, gate_type=None, batch_size=512):
    seed_path = os.path.join("tune_seed", f"best_weather_router_{pred_len}_cas.json")
...
def main():
    parser = argparse.ArgumentParser(description="Ablation HPO script for weather gating choices.")
    ...
    for p_len in args.pred_len:
        print(f"\n=== [MODE: HPO {args.gate_type.upper()}] Starting tuning for weather | pred_len={p_len} ===")
        bounds = make_bounds()
